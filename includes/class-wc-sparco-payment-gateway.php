...
class WC_Gateway_Sparco extends WC_Payment_Gateway {
	...
	public function check_response() {
		...
		$webhook_payload = (array) json_decode( $body );

		$webhook_payload = wp_unslash( $webhook_payload );

		if ( ! isset( $webhook_payload['merchantReference'] ) ) {
			exit;
		}
		...
		try {
			...
			if($webhook_payload_verified){
				...
				$order_id = (int) array_slice(explode('-',$merchant_ref), -1)[0];
	...
	private function sparco_payment_processing($order)
	{
			...
			$body = [
				...
				'customerAddr'=>$order->get_billing_address_1(),
			...
			if(200 === wp_remote_retrieve_response_code($response)){
				...
				if(isset( $res->paymentUrl)){
					$sparco_redirect_url = esc_url($res->paymentUrl);
				}
